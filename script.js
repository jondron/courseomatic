...
$(document).ready(function () {
    ...
    const specificActivities = {
        ...
        practice: ["exercises", "tests & quizzes", "drills", "games", "simulations & role plays", "other"],
        investigation: ["research project", "fieldwork", "case study", "problem-based learning", "data analysis", "experiment", "lab", "other"],
        reflection: ["journaling", "discussion", "portfolio", "exit takeaway", "reflective essay", "feedback", "survey", "other"],
        production: ["writing", "presentation", "drawing", "experiment", "coding", "prototyping", "model design", "concept map", "portfolio", "project", "other"],
    ...
    function initializeColourKey() {
        let colorKeyHtml = '<div id="color-key" style="display: flex; justify-content: space-around; padding: 10px 0; background-color: #f0f0f0; margin-bottom: 10px;">';
        colorKeyHtml += "<br />Activity type key: <br />";
        ...
        colorKeyHtml += '</div>';
        $('#color-key-container').html(colorKeyHtml);
        return colorKeyHtml;
    }
...
function generateNewActivityId() {
    ...
    function calculateDurations() {
        ...
        $('#total-course-duration').html(`Total Course Duration: ${Math.floor(totalCourseMinutes / 60)}h ${totalCourseMinutes % 60}m`);
    }
    ...
    $('#learning-activity-menu').on('change', function () {
        ...
        if (activityType) {
            showLearningActivityPopup(activityType);
        }
        $(this).val('');
    });
    ...
    function editUnit(unitIndex) {
        ...
        $('#popup').html(`
            <div style="padding: 20px;">
                ...
                <label>Unit Description: <textarea id="edit-unit-description" style="width: 90%; height: 60px;">${unit.description}</textarea></label><br><br>
        ...
        $('#save-unit').on('click', function () {
            ...
            unit.description = quill.root.innerHTML;
            renderUnits();
      		initializeActivitySortable(); // Re-initialize sortable after editing
      		 addActivityCardListeners(); // Add event listeners for activity cards
        	calculateDurations(); // Recalculate durations after editing
        	saveCourseToLocalStorage(); // Save the updated courseData to local storage
        	// console.log(courseData.units[unitIndex].description);
        	closePopup();
    ...
    function updateCourseHeader() {
		...
		const courseDetails=`
			<h3>${courseName}</h3>
			<strong>Credits:</strong> ${courseCreditHours} <br />
			<strong>Goal:</strong> ${courseGoal}<br /> 
			<strong>Description:</strong> ${courseDescription} <br />
			<strong><div id='total-course-duration'> Total Course Duration:</strong> ${totalDurationText}</div>
		...
		// Update the learning outcomes display
		$('#learning-outcomes-display').html(outcomesHtml);
	}
    ...
    function editActivity(unitIndex, activityIndex) {
        const activity = courseData.units[unitIndex].activities[activityIndex];
            ...
            <div style="padding: 20px;">
                ...
                <label>Duration (hours:minutes or minutes): <input type="text" id="edit-activity-duration" value="${activity.duration}" style="width: 90%;"></label><br><br>
   ...
   function editUnit(unitIndex) {
	...
	function showLearningActivityPopup(type) {
		if (!checkUnitsExist()) return; // Check for unit existence before proceeding
		...
		$('#popup').html(`
			<div style="padding: 20px;">
				...
				<label>Duration (hours:minutes): <input type="text" id="activity-duration" placeholder="00:00" style="width: 90%;"></label><br><br>
 ...
 let activityChartInstance; // Variable to hold the chart instance
   ...
   function saveCourseToPDF() {
	...
	function saveCourseToHTML() {
		let htmlContent = `<!DOCTYPE html>
			...
			<div id="color-key" style="display: flex; justify-content: space-around; padding: 10px 0; background-color: #f0f0f0; margin-bottom: 10px;">
				<!-- Include the color key HTML here if necessary -->
			</div>
			<h2>Units</h2>
		`;
  ...
  /*  $('body').prepend('<div id="total-course-duration" style="font-size: 16px; font-weight: bold;">Total Course Duration: 0h 0m</div>'); */
});
